...
#### 简介HBase
    HBase是在hdfs基础
    - 特点
        1. 每个值是一个未经解释的字符串，没有数据类型
        2. 用户在表中存储数据，每一行都有一个可排序行键和任意多的列
        3. 表在水平方向由一个或多个列族组成，一个列族中可以包含任意多个列，同一个列族里数据存储在一起
        4. 列族支持动态扩展，可轻松添加一个列族或列，无需预先定义类型数量，所有都以字符串形式存储
        5. HBase更新会生成新版本，保留旧版本
    - 一些定义
        1. 单元格：行，列族，列限定符确定一个单元格，数据存储在单元格里
        2. 列限定符：列族里数据通过列限定符（或列）定位
        3. 时间戳：每个单元格保存同一份数据各个版本，以时间戳索引
    - 定位特点<br>
